...
async def to_code(config):
    ...
    if max_distance_config := config.get(CONF_MAX_DISTANCE):
        # Bug 9 fix: distance params are gate numbers (1~16), not meters (0~8.4).
        # Each gate ≈ 0.7 m; 16 gates ≈ 11.2 m max range per protocol Table 2-2.
        n = await number.new_number(max_distance_config, min_value=1, max_value=16, step=1)
    ...
    if min_distance_config := config.get(CONF_MIN_DISTANCE):
        n = await number.new_number(min_distance_config, min_value=0, max_value=16, step=1)
        await cg.register_parented(n, config[CONF_LD2410S_ID])
