...
namespace esphome
{
    namespace ld2410s
    {
        ...
        void LD2410S::setup() {
            if (!this->set_config_mode(true)) {
                ESP_LOGE(TAG, "Failed to communicate with LD2410S sensor. Check wiring.");
                this->mark_failed();
                return;
            }
        ...
        bool LD2410S::set_config_mode(bool enabled) {
            CmdFrameT start_cfg;
            ...
            return this->send_command(start_cfg);
        }
        ...
        bool LD2410S::send_command(CmdFrameT frame)
        {
        ...
        void LD2410S::process_config_read_ack(uint8_t* data) {
            ...
            this->status_reporting_freq_number->publish_state(static_cast<float>(status_resp_freq) / 10.0f);
            this->distance_reporting_freq_number->publish_state(static_cast<float>(dist_resp_freq) / 10.0f);
#endif
#ifdef USE_SELECT
        ...
        CmdAckT LD2410S::parse_ack(uint8_t* buffer, size_t length) {
            ...
            for (size_t idx = 0; idx < result.length; idx++) {
                memcpy(&result.data[idx], &buffer[start + idx + 10], sizeof(buffer[start + idx + 10]));
            }
            return result;
        }
    }
